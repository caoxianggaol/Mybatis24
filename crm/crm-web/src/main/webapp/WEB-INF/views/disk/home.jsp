...
<script type="text/template" id="trTemplate">
   <%-- {{each data obj}}  obj.xxx--%>
    <%--https://aui.github.io/art-template/ 视屏crm-10 45分钟开始讲解JavaScript template  模板--%>
    <tr class="tr" rel="{{id}}">
...
<script>
    $(function () {
/*当disk不为空时取disk.id 否则取0 只要点进文件夹disk就有值*/
        ...
        //文件上传  Web Uploader组件添加静态资源 导入css和js
        var uploader = WebUploader.create({
            ...
            formData:{
                "pId":pid,
                "accountId":accountId
            } // formData是json对象 将键值对以 post方式发送请求给服务器的额外数据
        });

        var loadIndex = -1;
        //开始上传
        uploader.on('uploadStart',function (file) {
            loadIndex = layer.load(2);//进度条 2表示样式
        });
        //上传成功
